...
// We specialized it to support querying overlaps and gaps.
template<class V, int Order, class Allocator = allocator> requires(Order % 2 == 0)
class interval_btree {
  using K = decltype(declval<V>().begin);
...
private:
  ...
  void update_path_impl(node *n, K key) {
    if (!n)
      return;
  ...
  K find_gap_impl(node *n, size_t len, K min = 0) const {
  ...
  void find_overlap_impl(node *n, K start, K end, vector<V*> &result) const {
    if (!n || n->maxend <= start)
      return;
  ...
  bool has_overlap_impl(node *n, K start, K end) const {
    if (!n || n->maxend <= start)
      return false;
  ...
  static K id(const node *x) {
    return (K) x - 0xffff'ffff'c000'0000;
  }
...
public:
  ...
  interval_btree(const interval_btree &other) {
    root = copy_impl(other.root);
  }

  interval_btree &operator=(const interval_btree &other) {
    clear();
    root = copy_impl(other.root);
    return *this;
  }
  ...
  K find_gap(size_t len, size_t min = 0) const {
    return find_gap_impl(root, len, min);
  }

  void update_path(K key) {
    update_path_impl(root, key);
  }

  vector<V*> find_overlap(K start, K end) const {
    vector<V*> result;
    find_overlap_impl(root, start, end, result);
    return result;
  }

  bool has_overlap(K start, K end) const {
