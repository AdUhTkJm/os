...
但是，虚拟内存需要操纵页表，而操纵页表的代码几乎都默认页表的根是当前进程的 PCB 里所记载的那个：详见\cref{sec:pcb}。然而，PCB 中含有一个 \cpp{os::vector<vma>}，所以构造它需要分配虚拟内存。我有两个选择：将这个 vector 改为 intrusive\_list，或是增加一个 onboot 全局变量，对最开始的分配进行特殊处理。我选择了后者，因为 onboot 还有望在其他遇到类似困境的地方发挥作用。
...
因此，我们需要先 mount。我选择 mount 这三个文件系统：initramfs, tmpfs 和 devfs。这里的 initramfs 目前还是仅作调试用的。同时，记录一下支持的文件系统和 mount 它们的方式（\cref{subsec:vfs}）。
